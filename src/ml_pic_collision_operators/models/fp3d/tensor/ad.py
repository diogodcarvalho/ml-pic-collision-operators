...
class FokkerPlanck3D_Tensor_AD(FokkerPlanck3D_Base):
    """Fokker-Planck 3D Tensor Model.

    This model parametrizes A and D using 2 independent Tensors:

        A(vx, vy, vz) of shape (3, grid_size_x, grid_size_y, grid_size_z)
        D(vx, vy, vz) of shape (6, grid_size_x, grid_size_y, grid_size_z)
    ...
    def __init__(
        ...
        self.A = nn.Parameter(
            torch.zeros((3, grid_size[0], grid_size[1], grid_size[2]))
        )
        self.D = nn.Parameter(
            torch.zeros((6, grid_size[0], grid_size[1], grid_size[2]))
        )
