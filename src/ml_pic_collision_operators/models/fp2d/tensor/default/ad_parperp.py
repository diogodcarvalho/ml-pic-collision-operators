...
class FokkerPlanck2D_Tensor_AD_ParPerp(FokkerPlanck2D_Tensor_Base):
    ...
    This model parametrizes A_par, D_par, and D_perp using 3 independent Tensors:
        ...
        D_par(||v||) of shape (n_radial,)
        D_perp(||v||) of shape (n_radial,)
    ...
    Values of A_par, D_par, and D_perp are defined on a 1D grid of velocity magnitudes
    ...
    def __init__(
        ...
        self.Dperp = nn.Parameter(torch.zeros((self.n_radial)))
    ...
    def D_grid(self) -> torch.Tensor:
        ...
        Dxx = Dpar * self.cos_theta.data**2 + Dperp * self.sin_theta.data**2
        Dyy = Dpar * self.sin_theta.data**2 + Dperp * self.cos_theta.data**2
        Dxy = (Dpar - Dperp) * self.cos_theta.data * self.sin_theta.data

        return torch.stack([Dxx, Dyy, Dxy], dim=0)
