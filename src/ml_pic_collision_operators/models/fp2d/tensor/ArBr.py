...
class FokkerPlanck2D_ArBr(FokkerPlanck2DTensorBase):
    ...
    def __init__(
        ...
        self.B = nn.Parameter(torch.zeros((1, self.n_radial)))
    ...
    def B_grid(self) -> torch.Tensor:
        Bxx = torch_interpolate(self.vr_grid, self.vr_axis, self.B[0])
        Bxx = Bxx.reshape(*self.grid_size)
        return torch.stack([Bxx, Bxx, torch.zeros_like(Bxx)], dim=0)
