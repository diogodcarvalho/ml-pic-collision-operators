...
class FokkerPlanck2D_Arot_Bline(FokkerPlanck2DBase):
    """
    Fokker-Planck model whish assumes that:
        1. Ax = |A_r|*cos(theta)
        2. Ay = |A_r|*sin(theta)
        3. Bxx is symmetric along vx=0 and independent of vy
        4. Byy = Bxx^T
    ...
    def __init__(
        ...
        shape = (grid_size[0] // 2 + grid_size[0] % 2, 1)
    ...
    def __repr__(self):
        return (
            f"FokkerPlanck2D_Arot_Bline(A=Array{self.A.shape}, B=Array{self.B.shape}, "
            ...
            + f"n_radial={self.n_radial}, vr_axis=Array{self.vr_axis.shape}, vr_grid=Array{self.vr_grid.shape}), "
            + f"cos_theta=Array{self.cos_theta.shape}, sin_theta=Array{self.sin_theta.shape})"
        )
