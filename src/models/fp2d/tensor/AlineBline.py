...
class FokkerPlanck2D_AlineBline(FokkerPlanck2DTensorBase):
    ...
    def A_grid(self) -> torch.Tensor:
        ...
        Ax = Ax.repeat(1, self.grid_size[1])
    ...
    def B_grid(self) -> torch.Tensor:
        ...
        Bxx = Bxx.repeat(1, self.grid_size[1])
        Bxy = torch.concatenate(
            [self.B[1], -torch.flip(self.B[1], dims=(0,))[self.grid_size[0] % 2 :]],
            dim=0,
        )
        Bxy = Bxy.repeat(1, self.grid_size[0] // 2 + self.grid_size[0] % 2)
        Bxy = torch.concatenate(
