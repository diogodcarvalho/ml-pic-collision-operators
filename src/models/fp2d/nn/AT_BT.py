...
class FokkerPlanck2DNN_AT_BT(FokkerPlanck2DNNBase):
    """
    ...
    def B_grid(self) -> jax.Array:
        v = jax.lax.stop_gradient(self.v_grid)
        Bxx = jax.vmap(self.B[0])(v)
        Bxy = jax.vmap(self.B[1])(v)
        Bxx = Bxx.reshape(*self.grid_size)
        Bxy = Bxy.reshape(*self.grid_size)
        B_grid = jnp.stack([Bxx, Bxx.T, Bxy], axis=0)
        return B_grid
