...
class FokkerPlanck2DNNConditioned_AquadBquad(FokkerPlanck2DNNBaseConditioned):
    """
    This model parametrizes A_x, B_xx and Bxy using independet (equivalent) MLPs:

        A_x(vx, vy) = - sign(vx) * MLP_A_x(|vx|, |vy|, c)
    ...
    def A_grid(self, conditioners: torch.Tensor) -> torch.Tensor:
        # (batch_size * (grid_size//2 + grid_size%2)**2, 2 + C)
        inputs = self._prepare_input(conditioners)
        # print("i", inputs.shape)
        # (batch_size * (grid_size//2 + grid_size%2)**2, 1)
        Ax = self.Ax(inputs)
        # print("1", Ax.shape)
